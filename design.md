...
- 左侧栏：添加项目、搜索项目、成就墙入口、设置入口、最近项目列表。
- 顶部栏：侧栏切换、添加项目（侧栏收起时）、检测到新版本后出现的更新按钮、运行当前任务、任务清单、Markdown 预览。
...
- `src/main/updateService.ts`：基于 `electron-updater` 的手动更新检查、下载进度、安装重启和 IPC 状态广播。

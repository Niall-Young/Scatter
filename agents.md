...
- macOS 辅助功能权限引导弹窗必须等“偏好选择”弹窗结束后再出现，避免两个居中弹窗叠加；弹窗标题为“权限管理”，正文说明权限仅用于操作 Codex 客户端发送消息，底部只保留“重置权限”和“前往开启”，不要显示红色未授权提示。用户关闭弹窗后，本次会话不再因启动检查反复弹出，但运行前缺权限仍要拦截并展示。打开权限引导时 main process 会启动 `ScatterAccessibilityGuide` helper；helper 文案跟随 Scatter 当前语言设置，亮暗外观跟随 Scatter 当前主题设置，拖拽预览必须是完整 app cell 而不是被拉伸的图标；授权检测通过或用户关闭权限弹窗时要关闭 helper。

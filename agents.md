...
- Windows 不自动启动或操作 Codex/Claude；点击运行只生成当前范围 Markdown、复制到剪贴板，并提示用户手动粘贴到 Codex 或 Claude。Windows 不显示 macOS 辅助功能权限弹窗。
- 保持 `cwd` 指向当前项目文件夹。
...
- 最近项目保存在 Electron `userData`，不是每个项目里；已有项点击、打开和保存不改变顺序，新项目仍进顶部，用户拖整行排序后持久化到 `recent-projects.json`。
- 应用启动先显示独立无边框启动窗口；主窗口隐藏加载，ready 后也要等启动窗口至少显示 5 秒再打开。
- macOS 启动窗口和主窗口保留透明 Electron 窗口、背景模糊和带透明度的应用/画布背景色；Windows 启动窗口和主窗口使用不透明纯色背景。
...
- 最近项目列表项不显示通知徽标或 loading 图标状态；悬停或聚焦时显示“移除项目”图标按钮，只移除最近项目记录，不删除用户项目文件夹，也不能修改项目目录里的 `.scatter/scatter.json` 节点内容。启动、刷新最近项目和窗口重新聚焦时检查项目根文件夹；已删除的项目保留但置灰，禁用点击切换和拖拽，只能通过移除按钮清理。
...
- 画布缩放使用触控板捏合、`⌘` + 滚轮或缩放比例下拉菜单；画布平移使用触控板双指滑动、手形工具或空格键临时进入；框选使用 `⇧` + 拖拽，复制节点可按住 `⌥` 拖拽任意节点。节点右侧加号、以及没有上游节点时的左侧加号，拖线松开到空白画布会自动创建并连接下游或上游节点，且新节点连接点贴近松手点。选中连线后按 `Backspace` 或 `Delete` 删除该连线，方便重新连接父节点。画布快捷键包括 `⌘N` 新建节点、`⌘0` 定位画布、`V` 选择工具、`H` 手形工具。
